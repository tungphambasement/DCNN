...
template <typename T> struct TensorView<T, NHWC> {
  ...
  inline static void compute_strides(size_t *strides, const size_t *shape) {
    strides[0] = shape[1] * shape[2] * shape[3]; // stride for batch (N)
    strides[1] = 1;                              // stride for channels (C) - innermost
    strides[2] = shape[1] * shape[3];            // stride for height (H)
    strides[3] = shape[1];                       // stride for width (W)
  }
};
...
template <typename T> struct TensorView<T, NDHWC> {
  ...
  inline static void compute_strides(size_t *strides, const size_t *shape) {
    strides[0] = shape[1] * shape[2] * shape[3] * shape[4]; // stride for batch (N)
    strides[1] = 1;                                         // stride for channels (C) - innermost
    strides[2] = shape[1] * shape[3] * shape[4];           // stride for depth (D)
    strides[3] = shape[1] * shape[4];                      // stride for height (H)
    strides[4] = shape[1];                                 // stride for width (W)
  }
};
