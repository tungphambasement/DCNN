...
signed main() {
  ...
  Tensor<float> deserialized =
      tpipeline::BinarySerializer::deserialize_tensor<float>(serialized,
                                                             offset);
  ...
  for (size_t i = 0; i < tensor.size(); ++i) {
    assert(original_data[i] == deserialized_data[i] &&
           "Data mismatch after serialization");
  }
  std::cout
      << "Naive Serialization and deserialization successful and data matches!"
      << std::endl;
  ...
  auto deserialize_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(
          ...
          .count();
  std::cout << "Naive Serialization time: " << serialize_duration
            << " microseconds" << std::endl;
  std::cout << "Naive Deserialization time: " << deserialize_duration
            << " microseconds" << std::endl;
  std::cout << "Naive Serialized size: " << serialized.size() << " bytes"
            << std::endl;
  ...
  Tensor<float> protobuf_deserialized =
      tpipeline::GoogleProtobufSerializer::deserialize_tensor<float>(
          protobuf_serialized);
  ...
  std::cout << "Protobuf Serialization and deserialization successful and data "
               "matches!"
            << std::endl;
  auto protobuf_serialize_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(
          protobuf_serialize_end - protobuf_serialize_start)
          .count();
  auto protobuf_deserialize_duration =
      std::chrono::duration_cast<std::chrono::microseconds>(
          protobuf_deserialize_end - protobuf_deserialize_start)
          .count();
  std::cout << "Protobuf Serialization time: " << protobuf_serialize_duration
            << " microseconds" << std::endl;
  std::cout << "Protobuf Deserialization time: "
            << protobuf_deserialize_duration << " microseconds" << std::endl;
  std::cout << "Protobuf Serialized size: " << protobuf_serialized.size()
            << " bytes" << std::endl;

  // Size comparison
  std::cout << "Original tensor size: " << tensor.size() << " elements"
            << std::endl;
  std::cout << "Original tensor data: " << (tensor.size() * sizeof(float))
            << " bytes" << std::endl;
  std::cout << "Binary serialized: " << serialized.size()
            << " bytes (overhead: "
            << (serialized.size() - tensor.size() * sizeof(float)) << " bytes)"
            << std::endl;
  std::cout << "Protobuf serialized: " << protobuf_serialized.size()
            << " bytes (overhead: "
            << (protobuf_serialized.size() - tensor.size() * sizeof(float))
            << " bytes)" << std::endl;

  double binary_ratio =
      static_cast<double>(serialized.size()) / (tensor.size() * sizeof(float));
  double protobuf_ratio = static_cast<double>(protobuf_serialized.size()) /
