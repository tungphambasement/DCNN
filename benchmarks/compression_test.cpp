...
#include "pipeline/old_binary_serializer.hpp"
...
int main() {
  ...
  std::vector<uint8_t> serialized_data = tpipeline::BinarySerializer::serialize_tensor(tensor);
  auto compression_start = std::chrono::high_resolution_clock::now();
  std::vector<uint8_t> compressed_data = tpipeline::ZstdCompressor::compress(serialized_data, 3);
  ...
  std::vector<uint8_t> decompressed_data = tpipeline::ZstdCompressor::decompress(compressed_data);
  ...
  Tensor<float, NCHW> deserialized_tensor =
      tpipeline::BinarySerializer::deserialize_tensor<float>(decompressed_data, offset);
