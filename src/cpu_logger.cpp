...
static int get_nproc_visible() {
  ...
  if (read_text_file("/sys/fs/cgroup/cpu.max", cpu_max) && !cpu_max.empty()) {
    ...
    if (cpu_max.rfind("max", 0) != 0) {
      if (sscanf(cpu_max.c_str(), "%lld %lld", &quota, &period) == 2 &&
          quota > 0 && period > 0) {
...
static bool read_cgroup_v2_usage_usec(uint64_t &usec_out) {
  std::string s;
  if (!read_text_file("/sys/fs/cgroup/cpu.stat", s))
    return false;
...
static bool read_cpu_aggregate(CpuTimes &t) {
  ...
  if (cpu != "cpu")
    return false;
  f >> t.user >> t.nice_ >> t.system >> t.idle >> t.iowait >> t.irq >>
...
static double cpu_percent_host(double interval_sec) {
  CpuTimes a, b;
  if (!read_cpu_aggregate(a))
    return 0.0;
  std::this_thread::sleep_for(
      std::chrono::milliseconds((int)(interval_sec * 1000)));
  if (!read_cpu_aggregate(b))
    return 0.0;
...
static double cpu_percent_container(double interval_sec, int ncpus_eff,
