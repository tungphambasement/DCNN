...
bool CpuInfo::init_core_topology() {
  ...
  for (int cpu = 0; cpu < logical_cores_; ++cpu) {
    std::string core_id_path = "/sys/devices/system/cpu/cpu" +
  ...
  if (!unique_core_ids.empty()) {
    ...
    std::cout << "DEBUG: Detected " << physical_cores_
              << " physical cores from topology" << std::endl;
    std::cout << "DEBUG: Hyperthreading: "
              << (supports_hyperthreading_ ? "Yes" : "No") << std::endl;
  } else {
    // Fallback: try thread siblings
    std::ifstream core_siblings(
    ...
    if (core_siblings.is_open()) {
      ...
      int threads_per_core =
          std::count(siblings.begin(), siblings.end(), ',') + 1;
  ...
  if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
    ...
    if (GetLogicalProcessorInformation(buffer.data(), &bufferSize)) {
      ...
      std::cout << "DEBUG: Detected " << physical_cores_
                << " physical cores from Windows API" << std::endl;
      std::cout << "DEBUG: Hyperthreading: "
                << (supports_hyperthreading_ ? "Yes" : "No") << std::endl;
    } else {
...
bool CpuInfo::init_frequency_info() {
  ...
  if (cpuInfo[0] != 0) {              // Check if leaf is supported
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
      ...
      if (RegQueryValueEx(hKey, "~MHz", nullptr, nullptr, (LPBYTE)&frequency,
...
bool CpuInfo::init_memory_hierarchy() {
  ...
  if (numa_available() != -1) {
    ...
    for (int node = 0; node < memory_hierarchy_.numa_nodes; ++node) {
      ...
      int ret =
          numa_node_to_cpus(node, (unsigned long *)cpus->maskp, cpus->size);
...
bool CpuInfo::init_container_detection() {
  ...
  if (cpu_quota.is_open() && cpu_period.is_open()) {
    ...
    if (quota > 0 && period > 0) {
      container_cpu_limit_ =
          static_cast<int>((double)quota / period * logical_cores_);
    }
  }
...
bool CpuInfo::detect_pcore_ecore_topology() {
  ...
  for (int cpu = 0; cpu < logical_cores_; ++cpu) {
    ...
    std::string freq_path = "/sys/devices/system/cpu/cpu" +
                            std::to_string(cpu) + "/cpufreq/cpuinfo_max_freq";
  ...
  if (!core_max_freq.empty()) {
    ...
    double p_core_threshold =
        max_freq * 0.85; // P-cores should be within 15% of max
    ...
    for (const auto &[core_id, threads] : core_to_threads) {
      ...
      bool is_p_core = (freq >= p_core_threshold) &&
                       (threads.size() > 1); // P-cores have hyperthreading
...
bool CpuInfo::read_proc_stat_linux() {
  ...
  iss >> cpu_label >> current_times.user >> current_times.nice >>
      current_times.system >> current_times.idle >> current_times.iowait >>
  ...
  if (prev_cpu_times_.user != 0) {
    unsigned long long prev_idle =
        prev_cpu_times_.idle + prev_cpu_times_.iowait;
    unsigned long long idle = current_times.idle + current_times.iowait;

    unsigned long long prev_non_idle =
        prev_cpu_times_.user + prev_cpu_times_.nice + prev_cpu_times_.system +
        prev_cpu_times_.irq + prev_cpu_times_.softirq + prev_cpu_times_.steal;
    ...
    if (total_diff > 0) {
      ...
      user_utilization_ =
          100.0 * (current_times.user - prev_cpu_times_.user) / total_diff;
      system_utilization_ =
          100.0 * (current_times.system - prev_cpu_times_.system) / total_diff;
      iowait_utilization_ =
          100.0 * (current_times.iowait - prev_cpu_times_.iowait) / total_diff;
    }
  }
  ...
  for (int core = 0; core < logical_cores_; ++core) {
    if (std::getline(stat_file, line)) {
      ...
      core_iss >> core_label >> current_core_times[core].user >>
          current_core_times[core].nice >> current_core_times[core].system >>
          current_core_times[core].idle >> current_core_times[core].iowait >>
          current_core_times[core].irq >> current_core_times[core].softirq >>
          current_core_times[core].steal;
    }
  }

  // Calculate per-core utilization based on difference from previous sample
  if (!prev_core_times_.empty() &&
      prev_core_times_.size() == static_cast<size_t>(logical_cores_)) {
    for (int core = 0; core < logical_cores_; ++core) {
      unsigned long long prev_idle =
          prev_core_times_[core].idle + prev_core_times_[core].iowait;
      ...
      unsigned long long non_idle =
          current_core_times[core].user + current_core_times[core].nice +
      ...
      unsigned long long total_diff =
          (idle + non_idle) - (prev_idle + prev_non_idle);
      unsigned long long idle_diff = idle - prev_idle;

      if (total_diff > 0) {
        cores_[core].utilization_percent =
...
bool CpuInfo::read_thermal_linux() {
  ...
  for (int zone = 0; zone < 20; ++zone) {
    std::string temp_path =
        "/sys/class/thermal/thermal_zone" + std::to_string(zone) + "/temp";
    std::string type_path =
        "/sys/class/thermal/thermal_zone" + std::to_string(zone) + "/type";
        ...
        if (hwmon_name.find("coretemp") != std::string::npos ||
          ...
          if (temp_file.is_open()) {
            ...
            std::cout << "DEBUG: Found hwmon temperature (" << hwmon_name
                      << "): " << thermal_info_.current_temp_celsius << "°C"
                      << std::endl;
            break;
          }
        }
  ...
  if (!found_temp) {
    if (is_virtualized_ || is_containerized_) {
      ...
      // unavailable"
      thermal_info_.current_temp_celsius =
          -1.0; // Special value indicating unavailable
      std::cout
          << "DEBUG: Thermal monitoring unavailable in virtualized environment"
          << std::endl;
    }
  }
...
bool CpuInfo::read_frequencies_linux() {
  ...
  for (int core = 0; core < logical_cores_; ++core) {
    std::string freq_path = "/sys/devices/system/cpu/cpu" +
                            std::to_string(core) + "/cpufreq/scaling_cur_freq";
    std::ifstream freq_file(freq_path);
    ...
    std::string gov_path = "/sys/devices/system/cpu/cpu" +
                           std::to_string(core) + "/cpufreq/scaling_governor";
...
CoreInfo CpuInfo::get_core_info(int logical_core_id) const {
  if (logical_core_id >= 0 &&
      logical_core_id < static_cast<int>(cores_.size())) {
...
void CpuInfo::print_info() const {
  ...
  if (thermal_info_.current_temp_celsius >= 0) {
    std::cout << "Temperature: " << thermal_info_.current_temp_celsius << "°C"
              << std::endl;
  } else {
    std::cout << "Temperature: Not available (virtualized environment)"
              << std::endl;
  }

  std::cout << "Thermal Throttling: "
            << (thermal_info_.thermal_throttling ? "Yes" : "No") << std::endl;

  std::cout << "\n=== Environment ===" << std::endl;
  std::cout << "Containerized: " << (is_containerized_ ? "Yes" : "No")
            << std::endl;
...
bool CpuInfo::init_windows_wmi() {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
    ...
    bufferSize = sizeof(buffer);
    if (RegQueryValueEx(hKey, "Identifier", nullptr, nullptr, (LPBYTE)buffer,
                        &bufferSize) == ERROR_SUCCESS) {
...
bool CpuInfo::init_windows_frequency_wmi() {
  ...
  if (PdhOpenQuery(nullptr, 0, &hQuery) == ERROR_SUCCESS) {
    ...
    // compatibility)
    if (PdhAddCounterA(hQuery,
                       "\\Processor Information(_Total)\\Processor Frequency",
                       0, &hCounter) == ERROR_SUCCESS) {
      if (PdhCollectQueryData(hQuery) == ERROR_SUCCESS) {
        PDH_FMT_COUNTERVALUE counterValue;
        if (PdhGetFormattedCounterValue(hCounter, PDH_FMT_DOUBLE, nullptr,
...
bool CpuInfo::init_windows_container_detection() {
  ...
  if (RegOpenKeyEx(
          HKEY_LOCAL_MACHINE,
          "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Containers", 0,
          KEY_READ, &hKey) == ERROR_SUCCESS) {
    is_containerized_ = true;
    RegCloseKey(hKey);
  }
...
bool CpuInfo::update_windows_perfcounters() {
  ...
  if (!initialized) {
    ...
    if (PdhAddCounterA(hQuery, "\\Processor(_Total)\\% Processor Time", 0,
                       &hTotalCounter) != ERROR_SUCCESS) {
      PdhCloseQuery(hQuery);
      return false;
    }
    ...
    for (int i = 0; i < logical_cores_; ++i) {
      ...
      if (PdhAddCounterA(hQuery, counterPath.c_str(), 0, &hCoreCounters[i]) !=
          ERROR_SUCCESS) {
  ...
  PDH_FMT_COUNTERVALUE counterValue;
  if (PdhGetFormattedCounterValue(hTotalCounter, PDH_FMT_DOUBLE, nullptr,
                                  &counterValue) == ERROR_SUCCESS) {
    overall_utilization_ =
  ...
  for (int i = 0; i < logical_cores_; ++i) {
    if (hCoreCounters[i] &&
        PdhGetFormattedCounterValue(hCoreCounters[i], PDH_FMT_DOUBLE, nullptr,
                                    &counterValue) == ERROR_SUCCESS) {
      ...
      if (PdhCollectQueryData(hQuery) == ERROR_SUCCESS) {
        ...
        if (PdhGetFormattedCounterValue(hCounter, PDH_FMT_DOUBLE, nullptr,
          ...
          for (auto &core : cores_) {
            ...
            core.governor =
                "windows"; // Windows manages frequency automatically
          }
        }
      }
    }
