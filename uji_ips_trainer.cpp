...
public:
    static float compute_loss(const Tensor<float>& predictions, const Tensor<float>& targets, 
        ...
        if (output_size < 2) return 0.0f; // Need at least x,y coordinates
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            // Calculate squared Euclidean distance in real-world space
    ...
    static Tensor<float> compute_gradient(const Tensor<float>& predictions, const Tensor<float>& targets,
        ...
        if (output_size < 2) return gradient;
        
        // Get normalization statistics
        auto target_stds = data_loader.get_target_stds();
        if (target_stds.size() < 2) return gradient;
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            // Compute gradient in real-world space, then transform back to normalized space
            for (size_t j = 0; j < std::min(size_t(2), output_size); ++j) {
...
// Calculate positioning accuracy (for regression)
float calculate_positioning_accuracy(const Tensor<float>& predictions, const Tensor<float>& targets, 
                                   const WiFiDataLoader& data_loader, float threshold_meters = 5.0f) {
    const size_t batch_size = predictions.shape()[0];
    const size_t output_size = predictions.shape()[1];
    ...
    for (size_t i = 0; i < batch_size; ++i) {
        // Denormalize predictions and targets if needed
        std::vector<float> pred_coords(output_size), target_coords(output_size);
...
// Calculate average positioning error in meters (for regression)
float calculate_average_positioning_error(const Tensor<float>& predictions, const Tensor<float>& targets, 
    ...
    if (debug && batch_size > 0) {
        ...
        for (size_t i = 0; i < std::min(size_t(3), batch_size); ++i) {
            ...
            std::cout << "  Raw target: (" << target_coords[0] << ", " << target_coords[1] << ")" << std::endl;
            ...
            if (data_loader.is_normalized()) {
                ...
                std::cout << "  Denorm pred: (" << denorm_pred[0] << ", " << denorm_pred[1] << ")" << std::endl;
                std::cout << "  Denorm target: (" << denorm_target[0] << ", " << denorm_target[1] << ")" << std::endl;
...
float calculate_classification_accuracy(const Tensor<float>& predictions, const Tensor<float>& targets) {
    const size_t batch_size = predictions.shape()[0];
...
void train_ips_model(layers::Sequential<float>& model,
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        double total_positioning_error = 0.0; // Add tracking for positioning error in meters
        int num_batches = 0;
        ...
        while (train_loader.get_next_batch(batch_features, batch_targets)) {
            ...
            if (is_regression) {
                // Regression task (coordinate prediction) - use distance-based loss
                ...
                positioning_error = calculate_average_positioning_error(predictions, batch_targets, train_loader);
        ...
        const float avg_train_positioning_error = is_regression ? static_cast<float>(total_positioning_error / num_batches) : 0.0f;
        ...
        double val_positioning_error = 0.0; // Add validation positioning error tracking
        int val_batches = 0;
        ...
        while (test_loader.get_next_batch(batch_features, batch_targets)) {
            ...
            if (is_regression) {
                ...
                // Add debug output for first validation batch
                if (val_batches == 0) {
                    std::cout << "\nDEBUG: First validation batch analysis:" << std::endl;
                    val_positioning_error += calculate_average_positioning_error(predictions, batch_targets, test_loader, true);
                } else {
                    val_positioning_error += calculate_average_positioning_error(predictions, batch_targets, test_loader);
                }
            } else {
                apply_tensor_softmax(predictions);
        ...
        const float avg_val_positioning_error = is_regression ? static_cast<float>(val_positioning_error / val_batches) : 0.0f;
        ...
        if (is_regression) {
            std::cout << "Training   - Distance Loss: " << std::fixed << std::setprecision(2)
                      ...
                      << avg_train_accuracy * 100.0f << "%, Avg Error: " 
                      << std::setprecision(2) << avg_train_positioning_error << "m" << std::endl;
            std::cout << "Validation - Distance Loss: " << std::fixed << std::setprecision(2)
                      << avg_val_loss << "m², Accuracy (<5m): " << std::setprecision(2)
                      << avg_val_accuracy * 100.0f << "%, Avg Error: " 
                      << std::setprecision(2) << avg_val_positioning_error << "m" << std::endl;
        } else {
            std::cout << "Training   - CE Loss: " << std::fixed << std::setprecision(6)
                      << avg_train_loss << ", Accuracy: " << std::setprecision(2)
                      << avg_train_accuracy * 100.0f << "%" << std::endl;
            std::cout << "Validation - CE Loss: " << std::fixed << std::setprecision(6)
        ...
        if ((epoch + 1) % ips_constants::LR_DECAY_INTERVAL == 0) {
            ...
            std::cout << "Learning rate decayed: " << std::fixed << std::setprecision(8)
...
int main() {
    try {
        ...
        bool is_regression = true; // Change this based on your task
        WiFiDataLoader train_loader(is_regression), test_loader(is_regression);
        
        // Load data - adjust file paths and column indices for your dataset
        std::string train_file = "./data/uji/train.csv";  // Replace with your training file
        std::string test_file = "./data/uji/validation.csv";    // Replace with your test file
        
        std::cout << "\nLoading training data from: " << train_file << std::endl;
        // For UJI dataset: features are columns 0-519 (WAP001-WAP520), targets are columns 520-521 (LONGITUDE, LATITUDE)
        if (!train_loader.load_data(train_file, 0, 520, 520, 522, true)) {
            std::cerr << "Failed to load training data!" << std::endl;
            std::cerr << "Please ensure the data file exists and adjust column indices if needed." << std::endl;
            return -1;
        }
        ...
        // Get training data normalization statistics
        ...
        // Build model architecture with flat layers only
        ...
        model.print_summary(std::vector<size_t>{ips_constants::MAX_BATCH_SIZE, input_features, 1, 1});
        ...
        // Save model
        try {
            const std::string model_name = is_regression ? "ips_regression_model" : "ips_classification_model";
