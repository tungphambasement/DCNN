...
#if defined(USE_TBB)
inline std::unique_ptr<tbb::global_control> g_tbb_control; // global control for TBB threads
#endif
...
constexpr std::vector<EnumType> get_enum_vector() {
  ...
  static_assert(std::is_same_v<decltype(EnumType::_COUNT), EnumType>,
                "Enum type must have a _COUNT member to indicate the number of enum values");
  static_assert(std::is_same_v<decltype(EnumType::_START), EnumType>,
                "Enum type must have a _START member to indicate the starting enum value");
