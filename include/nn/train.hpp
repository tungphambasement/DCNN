...
ModelResult train_epoch(tnn::Sequential<float> &model,
...
ModelResult validate_model(tnn::Sequential<float> &model,
...
void train_classification_model(tnn::Sequential<float> &model,
  ...
  auto [best_val_loss, best_val_accuracy] = validate_model(model, test_loader);
  ...
  arena.execute([&] {
    ...
    for (int epoch = 0; epoch < config.epochs; ++epoch) {
      ...
      auto [avg_train_loss, avg_train_accuracy] = train_epoch(model, train_loader, config);
      ...
      auto [avg_val_loss, avg_val_accuracy] = validate_model(model, test_loader);
