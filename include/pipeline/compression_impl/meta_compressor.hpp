...
namespace tpipeline {
  static void internal_compress(const std::vector<uint8_t>& data, const std::string name){
    if(name == "zstd"){
  ...
  static void internal_decompress(const std::vector<uint8_t>& data, const std::string name){
    if(name == "zstd"){
  ...
  public:
    static std::vector<uint8_t> compress(const std::vector<uint8_t>& data, int clevel = 5, int shuffle = 1);
