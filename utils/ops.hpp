...
template <typename T>
T simd_dot_product_contiguous(const T *weights, const T *col_data,
                              size_t length, size_t weight_stride) {
  T sum = T(0);
  ...
  if constexpr (std::is_same_v<T, float>) {
    ...
    for (size_t i = 0; i < simd_end; i += 8) {
      ...
      __m256 w_vec = _mm256_set_ps(
          weights[(i + 7) * weight_stride], weights[(i + 6) * weight_stride],
          weights[(i + 5) * weight_stride], weights[(i + 4) * weight_stride],
          weights[(i + 3) * weight_stride], weights[(i + 2) * weight_stride],
          weights[(i + 1) * weight_stride], weights[i * weight_stride]);
